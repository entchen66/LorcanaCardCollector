...
def csv_to_json(csv_file_path):
    ...
    try:
        with open(csv_file_path, 'r', encoding='utf-8') as csv_file:
            ...
            csv_file.seek(0)
            try: dialect = csv.Sniffer().sniff(csv_file.read(2048), delimiters=',;\t|')
            except csv.Error: print("Warning: Could not automatically determine CSV delimiter. Assuming comma."); dialect = 'excel'
            csv_file.seek(0)
            csv_reader = csv.DictReader(csv_file, dialect=dialect)
            csv_reader.fieldnames = [key.strip().replace('\ufeff', '') for key in csv_reader.fieldnames or headers]
            return list(csv_reader)
    except FileNotFoundError: print(f"Error: CSV file not found at {csv_file_path}"); return []
    except Exception as e: print(f"Error reading CSV file {csv_file_path}: {e}"); return []
...
def calculate_multicolor_assignments(collection):
    ...
    for (chapter, color_pair), card_keys in cards_by_combo.items():
        ...
        for i, card_key in enumerate(sorted_card_keys):
            ...
            if i < split_point: MULTICOLOR_ASSIGNMENTS[assignment_key] = color1
            else: MULTICOLOR_ASSIGNMENTS[assignment_key] = color2
            total_assigned += 1
...
def merge_images(images, vertically, offset_merge, space_color=(255, 255, 255, 0), align='center'):
    ...
    if vertically:
        max_width = max(widths) if widths else 1; total_height = sum(heights) + (offset_merge * (len(images) - 1)) if heights else 1; new_im = Image.new('RGBA', (max_width, total_height), space_color); y_offset = 0
        for im in images:
            if align == 'left': x_position = 0
            elif align == 'right': x_position = max_width - im.size[0]
            else: x_position = (max_width - im.size[0]) // 2
            paste_im = im if im.mode == 'RGBA' else im.convert('RGBA'); new_im.paste(paste_im, (x_position, y_offset), paste_im); y_offset += im.size[1] + offset_merge
    else:
        max_height = max(heights) if heights else 1; total_width = sum(widths) + (offset_merge * (len(images) - 1)) if widths else 1; new_im = Image.new('RGBA', (total_width, max_height), space_color); x_offset = 0
        for im in images:
            if align == 'top': y_position = 0
            elif align == 'bottom': y_position = max_height - im.size[1]
            else: y_position = (max_height - im.size[1]) // 2
            paste_im = im if im.mode == 'RGBA' else im.convert('RGBA'); new_im.paste(paste_im, (x_offset, y_position), paste_im); x_offset += im.size[0] + offset_merge
    return new_im
...
def process_images(lang, chapter_list, generate_name,
    ...
    IMAGES_PER_ROW = img_per_row; all_images_per_chapter = defaultdict(list); total_processed_cards = 0
    font_count, font_chapter = None, None; normal_count_img_base, foil_count_img_base, done_img_base, missing_img_base = None, None, None, None
    try:
        font_count = ImageFont.truetype("assets/black.ttf", int(50 * SCALING)); font_chapter = ImageFont.truetype("assets/black.ttf", int(180 * SCALING)); normal_count_img_base = Image.open("assets/normal_card_count.png"); foil_count_img_base = Image.open("assets/foil_card_count.png"); done_img_base = Image.open("assets/done.png"); missing_img_base = Image.open("assets/missing.png") # noqa e501
    except Exception as e: print(f"Warning: Error loading assets/fonts: {e}. Some overlays/text might be missing."); font_count = ImageFont.load_default(); font_chapter = ImageFont.load_default() # noqa e501
    ...
    for chapter in chapter_list:
        ...
        for img_filename in os.listdir(chapter_dir):
            ...
            if target_color:
                ...
                if is_multicolor:
                    ...
                    if rarity == "ENCHANTED":
                        ...
                        if assigned_color == target_color_lower: include_card = True
            ...
            else: include_card = True # No target color

            if not include_card: continue
            # --- End Filtering Logic ---

            # --- Image loading & Metadata (Remains the same) ---
            total_processed_cards += 1; normal_count = int(card_info.get("normal", 0)); foil_count = int(card_info.get("foil", 0)); total_count = normal_count + foil_count # noqa e501
            img_path = os.path.join(chapter_dir, img_filename)
            try: img = Image.open(img_path).convert("RGBA")
            except Exception as e: print(f"Error opening image {img_path}: {e}"); continue
            overlay = None
            if total_count == 0: overlay = Image.new('RGBA', img.size, (155, 110, 110, 160))
            elif total_count >= 4 and mark_completed: overlay = Image.new('RGBA', img.size, (110, 155, 110, 160))
            if overlay: img = Image.alpha_composite(img, overlay)
            metadata = {"chapter": chapter, "card_number": card_key, "color": card_info["color"], "color_rgb": color_rgb, "filename": img_filename, "is_missing": total_count == 0, "normal_count": normal_count, "foil_count": foil_count, "total_count": total_count, "rarity": rarity} # noqa e501
    ...
    images_to_merge = []; total_missing_in_view = 0
    for chapter, images_with_metadata in all_images_per_chapter.items():
        if not images_with_metadata: continue
        num_images = len(images_with_metadata); rows = (num_images + IMAGES_PER_ROW - 1) // IMAGES_PER_ROW; grid_width = (CARD_WIDTH + PADDING) * min(IMAGES_PER_ROW, num_images) - PADDING + (2 * PADDING); grid_height = (CARD_HEIGHT + PADDING) * rows - PADDING + (2 * PADDING); chapter_image_grid = Image.new('RGBA', (grid_width, grid_height), (*color_rgb, 255)); x_offset, y_offset = PADDING, PADDING # noqa e501
        for index, (img, metadata) in enumerate(images_with_metadata):
            img_resized = img.resize((CARD_WIDTH, CARD_HEIGHT), resample=Image.LANCZOS); img_rounded = round_corners(img_resized, CORNER_RADIUS); chapter_image_grid.paste(img_rounded, (x_offset, y_offset), img_rounded) # noqa e501
            normal_count = metadata["normal_count"]; foil_count = metadata["foil_count"]; total_count = metadata["total_count"]
            if total_count > 0:
                 if normal_count_img_base and font_count: n_img = normal_count_img_base.copy(); n_img = n_img.resize((int(n_img.width * 0.75 * SCALING), int(n_img.height * 0.75 * SCALING)), resample=Image.LANCZOS); draw_n = ImageDraw.Draw(n_img); text_n = str(normal_count); bbox_n = draw_n.textbbox((0, 0), text_n, font=font_count); tx_n = (n_img.width - (bbox_n[2] - bbox_n[0])) // 2; ty_n = (n_img.height - (bbox_n[3] - bbox_n[1])) // 2 - int(10 * SCALING); draw_n.text((tx_n, ty_n), text_n, font=font_count, fill=(255, 255, 255)); chapter_image_grid.paste(n_img, (x_offset + CARD_WIDTH - n_img.width - int(n_img.width * 0.75) - 5, y_offset + 5), n_img) # noqa e501
                 if foil_count_img_base and font_count: f_img = foil_count_img_base.copy(); f_img = f_img.resize((int(f_img.width * 0.75 * SCALING), int(f_img.height * 0.75 * SCALING)), resample=Image.LANCZOS); draw_f = ImageDraw.Draw(f_img); text_f = str(foil_count); bbox_f = draw_f.textbbox((0, 0), text_f, font=font_count); tx_f = (f_img.width - (bbox_f[2] - bbox_f[0])) // 2; ty_f = (f_img.height - (bbox_f[3] - bbox_f[1])) // 2 - int(10 * SCALING); draw_f.text((tx_f, ty_f), text_f, font=font_count, fill=(255, 255, 255)); chapter_image_grid.paste(f_img, (x_offset + CARD_WIDTH - f_img.width - 5, y_offset + int(f_img.height * 0.75) + 5), f_img) # noqa e501
                 if total_count >= 4 and mark_completed and done_img_base: d_img = done_img_base.copy(); d_img = d_img.resize((int(d_img.width * 0.2 * SCALING), int(d_img.height * 0.2 * SCALING)), resample=Image.LANCZOS); chapter_image_grid.paste(d_img, (x_offset + int(15 * SCALING), y_offset + CARD_HEIGHT - d_img.height - int(15 * SCALING)), d_img) # noqa e501
            else: total_missing_in_view += 1
            if missing_img_base and total_count == 0 : m_img = missing_img_base.copy(); m_img = m_img.resize((int(m_img.width * 0.36 * SCALING), int(m_img.height * 0.36 * SCALING)), resample=Image.LANCZOS); chapter_image_grid.paste(m_img, (x_offset + CARD_WIDTH - m_img.width - int(5 * SCALING), y_offset + int(5 * SCALING)), m_img) # noqa e501
            x_offset += CARD_WIDTH + PADDING;
            if (index + 1) % IMAGES_PER_ROW == 0: x_offset = PADDING; y_offset += CARD_HEIGHT + PADDING # noqa e501
        if len(chapter_list) == 1 and font_chapter: chapter_name_image = Image.new('RGBA', (grid_width, int(210 * SCALING)), (*color_rgb, 255)); draw_title = ImageDraw.Draw(chapter_name_image); title_text = f"{CHAPTER_NAMES.get(chapter, chapter)}:"; title_color = (255, 255, 255); draw_title.text((PADDING, 5), title_text, font=font_chapter, fill=title_color); images_to_merge.append(chapter_name_image) # noqa e501
        images_to_merge.append(chapter_image_grid)

    # --- Final Image Saving (Remains the same) ---
    if not images_to_merge: print(f"No images generated for any chapter for {generate_name}."); return
    if len(images_to_merge) > 1: final_image = merge_images(images_to_merge, True, PADDING, (*color_rgb, 255), align="left")
    elif images_to_merge: final_image = images_to_merge[0]
    else: print(f"Error: No images available to save for {generate_name}"); return
    output_base_dir = os.path.join(BASE_DIR, output_subdir); sub_folder = "all_by_color" if target_color else "all_sets"; output_dir = os.path.join(output_base_dir, sub_folder, lang); os.makedirs(os.path.join(output_dir, "webp"), exist_ok=True); os.makedirs(os.path.join(output_dir, "png"), exist_ok=True) # noqa e501
    png_path = os.path.join(output_dir, "png", f"{generate_name}.png"); webp_path = os.path.join(output_dir, "webp", f"{generate_name}.webp") # noqa e501
    try:
        final_image.save(png_path, "PNG"); print(f"Image saved: {png_path}")
        try: final_image.save(webp_path, "WebP", quality=90)
        except Exception as e: print(f"Failed to save WebP image {webp_path}: {e}")
    except Exception as e: print(f"Failed to save PNG image {png_path}: {e}")
    if total_missing_in_view > 0: print(f"Total missing cards shown in {generate_name}: {total_missing_in_view}")
...
def merge_cards(lang, color_rgb, img_per_row, generate_name, mark_completed, chapter_list):
    ...
    process_images(lang=lang, chapter_list=chapter_list, generate_name=generate_name, target_color=None, multicolor_assignments=MULTICOLOR_ASSIGNMENTS, img_per_row=img_per_row, color_rgb=color_rgb, mark_completed=mark_completed, output_subdir="output")
...
def merge_cards_for_color(lang, merge_color, color_rgb, img_per_row, generate_name, mark_completed, chapter_list):
    ...
    process_images(lang=lang, chapter_list=chapter_list, generate_name=generate_name, target_color=merge_color, multicolor_assignments=MULTICOLOR_ASSIGNMENTS, img_per_row=img_per_row, color_rgb=color_rgb, mark_completed=mark_completed, output_subdir="output")
...
def merge_cards_missing_for_playset(lang, img_per_row, generate_name, chapter_list, rarity_filter=None):
    ...
    IMAGES_PER_ROW = img_per_row; bg_color = (255, 255, 255); text_color = (0, 0, 0); all_images_per_chapter = defaultdict(list); total_cards_needed = 0
    font_missing_count, font_chapter_missing = None, None; count_img_base = None
    try:
        font_missing_count = ImageFont.truetype("assets/black.ttf", int(100 * SCALING)); font_chapter_missing = ImageFont.truetype("assets/black.ttf", int(180 * SCALING)); count_img_base = Image.open("assets/foil_card_count.png") # noqa e501
    except Exception as e: print(f"Warning: Error loading assets/fonts for missing playset: {e}. Overlays/text may be missing."); font_missing_count = ImageFont.load_default(); font_chapter_missing = ImageFont.load_default() # noqa e501
    ...
    for chapter in chapter_list:
        ...
        for img_filename in os.listdir(chapter_dir):
            ...
            # Check playset count
            normal_count = int(card_info.get("normal", 0)); foil_count = int(card_info.get("foil", 0)); total_count = normal_count + foil_count # noqa e501
            if total_count >= 4: continue
            # --- End Filtering Logic ---

            # --- Image Loading & Metadata (Remains the same) ---
            missing_count = 4 - total_count; total_cards_needed += missing_count
            img_path = os.path.join(chapter_dir, img_filename)
            try: img = Image.open(img_path).convert("RGBA")
            except Exception as e: print(f"Error opening image {img_path}: {e}"); continue
            metadata = {"chapter": chapter, "card_number": card_key, "filename": img_filename, "missing_count": missing_count, "rarity": card_info.get("rarity", "")} # noqa e501
            chapter_cards.append((img, metadata))
            # --- End Image Loading & Metadata ---
    ...
    for chapter, images_with_metadata in all_images_per_chapter.items():
        if not images_with_metadata: continue
        num_images = len(images_with_metadata); rows = (num_images + IMAGES_PER_ROW - 1) // IMAGES_PER_ROW; grid_width = (CARD_WIDTH + PADDING) * min(IMAGES_PER_ROW, num_images) - PADDING + (2 * PADDING); grid_height = (CARD_HEIGHT + PADDING) * rows - PADDING + (2 * PADDING); chapter_image_grid = Image.new('RGBA', (grid_width, grid_height), (*bg_color, 255)); x_offset, y_offset = PADDING, PADDING # noqa e501
        for index, (img, metadata) in enumerate(images_with_metadata):
            img_resized = img.resize((CARD_WIDTH, CARD_HEIGHT), resample=Image.LANCZOS); img_rounded = round_corners(img_resized, CORNER_RADIUS); chapter_image_grid.paste(img_rounded, (x_offset, y_offset), img_rounded) # noqa e501
            missing_count = metadata["missing_count"]
            if count_img_base and font_missing_count: c_img = count_img_base.copy(); c_img = c_img.resize((int(c_img.width * 1.5 * SCALING), int(c_img.height * 1.5 * SCALING)), resample=Image.LANCZOS); draw_c = ImageDraw.Draw(c_img); text_c = str(missing_count); bbox_c = draw_c.textbbox((0, 0), text_c, font=font_missing_count); tx_c = (c_img.width - (bbox_c[2] - bbox_c[0])) // 2; ty_c = (c_img.height - (bbox_c[3] - bbox_c[1])) // 2 - int(10 * SCALING); draw_c.text((tx_c, ty_c), text_c, font=font_missing_count, fill=(255, 255, 255)); chapter_image_grid.paste(c_img, (x_offset + CARD_WIDTH - c_img.width - 5, y_offset + 5), c_img) # noqa e501
            x_offset += CARD_WIDTH + PADDING;
            if (index + 1) % IMAGES_PER_ROW == 0: x_offset = PADDING; y_offset += CARD_HEIGHT + PADDING # noqa e501
        if font_chapter_missing: chapter_name_image = Image.new('RGBA', (grid_width, int(210 * SCALING)), (*bg_color, 255)); draw_title = ImageDraw.Draw(chapter_name_image); title_text = f"{CHAPTER_NAMES.get(chapter, chapter)}:"; draw_title.text((PADDING, 5), title_text, font=font_chapter_missing, fill=text_color); images_to_merge.append(chapter_name_image) # noqa e501
        images_to_merge.append(chapter_image_grid)
    ...
    output_dir = os.path.join(BASE_DIR, "output", "missing_playset", lang); os.makedirs(os.path.join(output_dir, "webp"), exist_ok=True); os.makedirs(os.path.join(output_dir, "png"), exist_ok=True) # noqa e501
    png_path = os.path.join(output_dir, "png", f"{generate_name}.png"); webp_path = os.path.join(output_dir, "webp", f"{generate_name}.webp") # noqa e501
    try:
        final_image.save(png_path, "PNG"); print(f"Image saved: {png_path}")
        try: final_image.save(webp_path, "WebP", quality=90)
        ...
        for chapter in process_chapters:
            ...
            for ct in CARD_TYPES:
                merge_cards_for_color(
                    lang=lang, merge_color=ct.name, color_rgb=ct.color,
                    img_per_row=9, generate_name=f"{chapter}_{ct.name}",
                    mark_completed=True, chapter_list=[chapter]
                )
